...
import { useBillingStore } from '../../billing/store/billingStore.js';
...
export const Orders = () => {
  ...
  const handleInvoice = (order) => {
    openConfirm({
      ...
      message: `¿Crear factura para la orden ${order._id}?`,
      onConfirm: async () => {
        const res = await createBill(order._id);
        if (res.ok) {
          showError('Factura generada correctamente');
        }
      },
    });
  };
            ...
            visibleOrders.map((order) => (
              <div key={order._id} className='bg-white rounded-xl shadow-sm border border-gray-200 p-5'>
                <div className='flex flex-col gap-3 md:flex-row md:items-start md:justify-between'>
                  <div>
                    <h2 className='text-xl font-semibold text-main-blue'>Orden #{order._id?.slice(-6)}</h2>
                    <p className='text-sm text-gray-500 mt-1'>
                      Mesa {order.tableId || 'N/A'} • {order.waiterName || order.waiterId}
                    </p>
                  </div>
                    ...
                    <ul className='space-y-2'>
                      {order.items?.map((item, index) => (
                        <li key={index} className='rounded-lg bg-slate-50 p-3'>
                          ...
                          <p className='text-sm text-gray-500'>Q{item.subtotal?.toFixed(2) ?? '0.00'}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
